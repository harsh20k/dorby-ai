...
``baselines/voyage_nano_field_selected/text.py`` / the LLM-judge focused
prompt's field lists — same rationale as that module's docstring: each
package's isolation is worth a few duplicated lines rather than a
cross-package import that couples an embedding baseline's text packing to
an LLM-judge package's evolution.
"""
