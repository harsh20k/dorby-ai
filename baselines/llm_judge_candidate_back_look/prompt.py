"""Prompt + field-trimmed text serialization: candidate = background + lookingFor.
...
data-science experiments" isolation rule in CLAUDE.md. Seeker fields are
held fixed at ``positioning`` + ``lookingFor`` (same as the focused
experiment); the candidate field set is trimmed to two of its original
three fields — ``background`` + ``lookingFor`` (drops ``positioning``) —
to test candidate-side field permutations. The searchQuery and system
prompt text are unchanged.

Pushed to LangSmith Prompt Hub as ``llm_judge_candidate_back_look`` (see
``scripts/push_llm_judge_candidate_back_look_prompt.py``); the string here
is the source of truth pushed from, not pulled at eval time.
"""
...
def seeker_to_text(user_contact_file: Mapping[str, Any]) -> str:
    """Seeker text = positioning + lookingFor, no searchQuery (added separately)."""
...
PROMPT_NAME = "llm_judge_candidate_back_look"
