"""Prompt + field-trimmed text serialization for the seeker-lookingFor-only variant.
...
``positioning`` + ``lookingFor`` down to ``lookingFor`` alone, to isolate
how much of the focused prompt's result comes from ``lookingFor`` specifically
versus ``positioning``. Candidate fields, the searchQuery, and the system
prompt text are all unchanged.

Pushed to LangSmith Prompt Hub as ``llm_judge_lookingfor_only`` (see
``scripts/push_llm_judge_lookingfor_only_prompt.py``); the string here is the
source of truth pushed from, not pulled at eval time.
"""
...
def seeker_to_text(user_contact_file: Mapping[str, Any]) -> str:
    """Seeker text = lookingFor only, no searchQuery (added separately)."""
...
PROMPT_NAME = "llm_judge_lookingfor_only"
