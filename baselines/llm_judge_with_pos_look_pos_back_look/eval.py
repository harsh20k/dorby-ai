"""Focused-field LLM-judge experiment: positioning/lookingFor/background + query.

A variation on ``baselines/llm_judge`` (see that package's docstring for the
original "no query, complete profile" experiment). This variant:

  * gives the model the seeker's searchQuery,
  * shows only positioning + lookingFor for the seeker and
    positioning + background + lookingFor for the candidate (not the full
    profile),
  * asks the model to reason about complementary need/supply and two-way
    preference fit specifically.

Scored the same way as the original — pair ROC-AUC/decision metrics against
the real human accept/decline outcome on the 200 real seed pairs — so the
two are directly comparable on the pair-metrics axis, with the caveat that
this variant is not "no query" anymore (see the module docstring in
``baselines/llm_judge_with_pos_look_pos_back_look/__init__.py``).

    python -m baselines.llm_judge_with_pos_look_pos_back_look.eval --data-dir data --env-file .env
    python -m baselines.llm_judge_with_pos_look_pos_back_look.eval --split holdout --env-file .env
"""
...
from baselines.llm_judge_with_pos_look_pos_back_look.prompt import (
    SYSTEM_PROMPTS,
    build_user_prompt,
    candidate_to_text,
    seeker_to_text,
...
def run_eval(
    *,
    ...
    return {
        "model_name": model,
        "experiment": "llm_judge_with_pos_look_pos_back_look",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.backend == "openrouter":
        ...
        if not api_key:
            ...
            return 2
        from baselines.llm_judge_with_pos_look_pos_back_look.google_backend import (
            call_google_verdict,
        )
    ...
    artifacts_dir = args.artifacts_dir or Path(
        "artifacts/llm_judge_with_pos_look_pos_back_look"
