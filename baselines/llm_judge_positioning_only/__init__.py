"""LLM-judge variant: seeker trimmed to positioning only, isolated experiment.
...
``positioning`` alone, to isolate that field's individual contribution to
the focused prompt's result. Candidate fields (``positioning`` +
``background`` + ``lookingFor``), the searchQuery, and the system prompt
