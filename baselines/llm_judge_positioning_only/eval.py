"""Seeker-positioning-only LLM-judge experiment: field-isolation of the focused prompt.
...
positioning alone, to isolate that field's individual contribution.
Candidate fields (positioning + background + lookingFor), the searchQuery,
and the system prompt are all unchanged from the focused experiment.
...
three are directly comparable on the pair-metrics axis.
    ...
    python -m baselines.llm_judge_positioning_only.eval --data-dir data --env-file .env
    python -m baselines.llm_judge_positioning_only.eval --split holdout --env-file .env
"""
...
from baselines.llm_judge_positioning_only.prompt import (
    SYSTEM_PROMPTS,
    build_user_prompt,
    candidate_to_text,
    seeker_to_text,
...
def run_eval(
    *,
    ...
    return {
        "model_name": model,
        "experiment": "llm_judge_positioning_only",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.backend == "openrouter":
        ...
        if not api_key:
            ...
            return 2
        from baselines.llm_judge_positioning_only.google_backend import (
            call_google_verdict,
        )
    ...
    artifacts_dir = args.artifacts_dir or Path(
        "artifacts/llm_judge_positioning_only"
