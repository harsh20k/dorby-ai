"""Eval for voyage-4-nano with lookingFor split into per-section candidate texts.

Same encoder, seeker text, and metrics shape as baselines/voyage_nano/eval.py;
only the candidate side changes: each candidate becomes N section-texts (full
profile, lookingFor swapped to one section at a time), and a candidate's score
against a seeker is the max cosine over its own sections rather than one
averaged whole-field embedding. See baselines/voyage_nano_sectioned/text.py.
"""
...
from baselines.voyage_nano_sectioned.text import candidate_to_sectioned_texts
...
def build_sectioned_corpus(
    positives: list[dict[str, Any]],
    negatives: list[dict[str, Any]],
) -> tuple[list[str], list[int], list[str]]:
    """Unique matchContactIds -> flattened section texts + per-candidate offsets.

    offsets has len(corpus_ids) + 1 entries; candidate i's sections occupy
    section_texts[offsets[i]:offsets[i + 1]].
    """
    ids: list[str] = []
    offsets: list[int] = [0]
    ...
    for record in positives + negatives:
        ...
        seen.add(mid)
        ids.append(mid)
        sections = candidate_to_sectioned_texts(record["matchContactFile"])
        section_texts.extend(sections)
        offsets.append(len(section_texts))
    return ids, offsets, section_texts
...
def run_eval(
    ...
    *,
    ...
    # Only used for the neg-hardness lexical-overlap slice, unaffected by sectioning.
    neg_cand_texts = [candidate_to_text(r["matchContactFile"]) for r in negatives]

    corpus_ids, offsets, section_texts = build_sectioned_corpus(positives, negatives)
    id_to_idx = {cid: i for i, cid in enumerate(corpus_ids)}
    print(f"candidate corpus size: {len(corpus_ids)} ({len(section_texts)} sections total)")
    ...
    pos_seeker_emb = encoder.encode(
        pos_seeker_texts, role="query", batch_size=batch_size, cache_name=None
    )
    neg_seeker_emb = encoder.encode(
        neg_seeker_texts, role="query", batch_size=batch_size, cache_name=None
    )
    section_emb = encoder.encode(
        section_texts, role="document", batch_size=batch_size, cache_name=None
    )

    # (n_queries x n_sections) -> max-pool per contiguous candidate block -> (n_queries x n_candidates)
    section_offsets = np.array(offsets[:-1])
    pos_section_scores = pos_seeker_emb @ section_emb.T
    neg_section_scores = neg_seeker_emb @ section_emb.T
    pos_candidate_scores = np.maximum.reduceat(pos_section_scores, section_offsets, axis=1)
    neg_candidate_scores = np.maximum.reduceat(neg_section_scores, section_offsets, axis=1)

    pos_target_idx = [id_to_idx[r["matchContactId"]] for r in positives]
    neg_target_idx = [id_to_idx[r["matchContactId"]] for r in negatives]

    pos_scores = np.array(
        [pos_candidate_scores[i, idx] for i, idx in enumerate(pos_target_idx)]
    )
    neg_scores = np.array(
        [neg_candidate_scores[i, idx] for i, idx in enumerate(neg_target_idx)]
    )
    ...
    for i, target_id in enumerate(pos_target_ids):
        order = np.argsort(-pos_candidate_scores[i], kind="stable")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Voyage-4-nano, lookingFor split into sections")
    ...
    p.add_argument("--artifacts-dir", type=Path, default=Path("artifacts/voyage_nano_sectioned"))
