"""Modal GPU entrypoint for the rrf_003-calibrated reciprocal baseline.

Thin wrapper around ``baselines.reciprocal_static_rrf003.eval.run_eval`` — same
code, same metrics, run on a Modal GPU instead of local MPS. Mirrors
``baselines/reciprocal_static/modal_eval.py``, plus mounting
``exports/rrf_datasets/rrf_003`` (the lambda-fitting population).

Usage:
  modal run baselines/reciprocal_static_rrf003/modal_eval.py
  modal volume get dorby-reciprocal-static-rrf003-eval real200 ./artifacts/reciprocal_static_rrf003
"""
...
APP_NAME = "dorby-reciprocal-static-rrf003-eval"
RESULTS_VOLUME = "dorby-reciprocal-static-rrf003-eval"
...
image = (
    ...
    .add_local_dir("exports/rrf_datasets/rrf_003", remote_path="/root/exports/rrf_datasets/rrf_003")
)
...
def eval_remote(
    ...
    from baselines.reciprocal_static_rrf003.eval import run_eval
    ...
    metrics = run_eval(
        ...
        rrf003_dir=Path("/root/exports/rrf_datasets/rrf_003"),
        model_name=model,
...
def main(
    ...
    """CLI: modal run baselines/reciprocal_static_rrf003/modal_eval.py"""
    call = eval_remote
    if gpu and gpu != "A10G":
    ...
    print("=== Modal reciprocal-static-rrf003 eval finished ===")
    lf = metrics["lambda_fit"]
    print(
        f"fitted lambda: {lf['fitted_lambda']} "
        f"(rrf_003 AUC fwd-only={lf['rrf003_auc_forward_only']:.4f} combined={lf['rrf003_auc_combined']:.4f})"
    )
    s = metrics["real200"]
    print(
        f"real200: pair AUC fwd-only={s['pair_forward_only']['roc_auc']:.4f} "
    ...
    print(
        ...
        f"./artifacts/reciprocal_static_rrf003_{run_id}"
    )
