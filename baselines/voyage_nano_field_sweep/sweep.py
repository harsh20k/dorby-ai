"""Run all 98 field/query combinations against real Voyage-4-nano encodes.

Encoding is the expensive step, so this dedups encode calls across combos
instead of doing one full encode pass per combo (98x):

- Candidate-side embeddings (pos/neg candidate texts + the retrieval corpus)
  depend only on ``candidate_fields`` -> 7 unique encode groups, not 98.
- Seeker-side embeddings depend only on (``seeker_fields``, ``use_query``)
  -> 7 x 2 = 14 unique encode groups, not 98.

That's 7 + 14 = 21 unique encode groups (each needing 2-3 encode calls:
pos/neg, plus corpus for candidates) feeding all 98 combos' pair/retrieval/
slice metrics, computed from cached embeddings.
...
from baselines.voyage_nano_field_sweep.fields import Combo, all_combos, subset_id
from baselines.voyage_nano_field_sweep.text import candidate_to_text, seeker_to_text
...
def run_sweep(
    *,
    ...
    # --- seeker-side: unique per (seeker_fields, use_query) (14 groups) ---
