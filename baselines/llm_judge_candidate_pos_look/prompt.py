"""Prompt + field-trimmed text serialization: candidate = positioning + lookingFor.
...
three fields — ``positioning`` + ``lookingFor`` (drops ``background``) —
to test candidate-side field permutations. The searchQuery and system
prompt text are unchanged.

Pushed to LangSmith Prompt Hub as ``llm_judge_candidate_pos_look`` (see
``scripts/push_llm_judge_candidate_pos_look_prompt.py``); the string here is
...
def candidate_to_text(match_contact_file: Mapping[str, Any]) -> str:
    """Candidate text = positioning + lookingFor only."""
    return _fields_to_text(match_contact_file, CANDIDATE_FIELDS)


PROMPT_NAME = "llm_judge_candidate_pos_look"
