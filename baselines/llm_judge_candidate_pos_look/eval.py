"""Candidate-field-permutation LLM-judge experiment: candidate = positioning + lookingFor.
...
side drops to two of its original three fields — positioning + lookingFor
(no background) — to test candidate-side field permutations. searchQuery
...
axis.
    ...
    python -m baselines.llm_judge_candidate_pos_look.eval --data-dir data --env-file .env
    python -m baselines.llm_judge_candidate_pos_look.eval --split holdout --env-file .env
"""
...
from baselines.llm_judge_candidate_pos_look.prompt import (
    SYSTEM_PROMPTS,
    build_user_prompt,
    candidate_to_text,
    seeker_to_text,
...
def run_eval(
    *,
    ...
    return {
        "model_name": model,
        "experiment": "llm_judge_candidate_pos_look",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.backend == "openrouter":
        ...
        if not api_key:
            ...
            return 2
        from baselines.llm_judge_candidate_pos_look.google_backend import (
            call_google_verdict,
        )
    ...
    artifacts_dir = args.artifacts_dir or Path(
        "artifacts/llm_judge_candidate_pos_look"
