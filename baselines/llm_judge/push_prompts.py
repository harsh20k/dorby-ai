"""Push the ``structured_cot`` judge prompt to LangSmith Hub.

The ``naive``/``calibrated`` variants are plain Python strings in ``prompt.py``
and were never versioned this way. ``structured_cot`` is different enough (a
new, unreviewed multi-aspect schema) that its source of truth lives in
``prompts/structured_cot.md`` and gets pushed here first, so the exact prompt
that produced any run's numbers is a named, inspectable hub commit rather than
just "whatever eval.py imported that day."

Usage::

    python -m baselines.llm_judge.push_prompts --dry-run
    python -m baselines.llm_judge.push_prompts --tag v1
"""
...
from baselines.llm_judge.prompt import PROMPTS_DIR

DEFAULT_HUB_REPO = "llm-judge-structured-cot"
LOCAL_FILE = "structured_cot.md"


def _escape_braces(text: str) -> str:
    """Brace-escape the JSON schema example so ChatPromptTemplate doesn't treat
    ``{name}``/``{score}`` as template variables — there are none in this prompt."""
...
def push(*, owner: str | None, tag: str | None, description: str | None, dry_run: bool) -> str:
    ...
    kwargs: dict = {
        "object": prompt,
        "description": description or f"dorby-ai llm_judge structured_cot (from {LOCAL_FILE})",
    }
    if tag:
