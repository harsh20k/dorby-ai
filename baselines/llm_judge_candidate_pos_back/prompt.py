"""Prompt + field-trimmed text serialization: candidate = positioning + background.
...
three fields — ``positioning`` + ``background`` (drops ``lookingFor``) —
to test candidate-side field permutations. The searchQuery and system
prompt text are unchanged.

Pushed to LangSmith Prompt Hub as ``llm_judge_candidate_pos_back`` (see
``scripts/push_llm_judge_candidate_pos_back_prompt.py``); the string here is
...
def candidate_to_text(match_contact_file: Mapping[str, Any]) -> str:
    """Candidate text = positioning + background only."""
    return _fields_to_text(match_contact_file, CANDIDATE_FIELDS)


PROMPT_NAME = "llm_judge_candidate_pos_back"
