"""Lambda sensitivity sweep for the static reciprocal score, on voyage_gemini_ctrl.
...
``voyage_gemini_ctrl_001`` LoRA adapter (baselines/reciprocal_lambda_grid_voyage_gemini_ctrl/
encode.py::VoyageGeminiCtrlEncoder) instead of frozen voyage-4-nano. Same mechanics
...
Usage:
  python -m baselines.reciprocal_lambda_grid_voyage_gemini_ctrl.eval --data-dir data
"""
...
from baselines.reciprocal_lambda_grid_voyage_gemini_ctrl.encode import (
    VOYAGE_GEMINI_CTRL_ADAPTER_DIR,
    VOYAGE_GEMINI_CTRL_MAX_SEQ_LENGTH,
    VOYAGE_GEMINI_CTRL_TRUNCATE_DIM,
    VoyageGeminiCtrlEncoder,
    cosine_scores,
)
from baselines.reciprocal_lambda_grid_voyage_gemini_ctrl.text import bg_text, look_text, seeker_look_text
...
def run_eval(
    ...
    encoder = VoyageGeminiCtrlEncoder(
    ...
    return {
        "model_name": "voyageai/voyage-4-nano + voyage_gemini_ctrl_001 LoRA adapter",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Lambda sensitivity sweep on voyage_gemini_ctrl (no fitting)")
    p.add_argument("--data-dir", type=Path, default=Path("data"))
    p.add_argument("--split-path", type=Path, default=Path("data/synthetic/seed_split.json"))
    p.add_argument("--adapter-dir", type=Path, default=VOYAGE_GEMINI_CTRL_ADAPTER_DIR)
    p.add_argument("--batch-size", type=int, default=4)
    p.add_argument("--max-seq-length", type=int, default=VOYAGE_GEMINI_CTRL_MAX_SEQ_LENGTH)
    p.add_argument("--truncate-dim", type=int, default=VOYAGE_GEMINI_CTRL_TRUNCATE_DIM)
    p.add_argument("--artifacts-dir", type=Path, default=Path("artifacts/reciprocal_lambda_grid_voyage_gemini_ctrl"))
