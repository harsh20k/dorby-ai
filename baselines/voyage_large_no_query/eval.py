"""Offline eval for Voyage-4-large API — seeker profile only (no searchQuery)."""
...
from baselines.voyage_large.eval import build_candidate_corpus, load_pairs
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Voyage-4-large API bi-encoder baseline eval (no searchQuery on seeker)"
    )
