"""Disk-cached encoder wrapper around the fine-tuned ``voyage_gemini_ctrl_001`` checkpoint.

``voyage_gemini_ctrl_001`` (``twotower_voyage_gemini_ctrl``) is `top1_ctrl`'s
exact LoRA recipe (rank 8, alpha 16, dropout 0.05, q/k/v/o_proj on frozen
Voyage-4-nano) retrained on a bigger, newer synthetic batch
(``pairing_voyage_gemini/smoke_test_002``, 3,008 rows vs. `top1_ctrl`'s 643)
with unchanged full-profile text on both sides. All-200: pair AUC 0.6081 —
the best pair AUC of any fine-tune in the project — but behind
`queryonly_back_look_001` on hard-neg AUC/MRR/R@1/R@10 (see this package's
``__init__.py`` and `docs/twotower-voyage-gemini-ctrl-experiment.md`).
...
VOYAGE_GEMINI_CTRL_ADAPTER_DIR = Path(
