"""Run all 105 field/query combinations against the fine-tuned queryonly_back_look checkpoint.

Same sweep design as ``baselines/twotower_top1_ctrl_field_sweep/sweep.py``
(and, before it, ``baselines/voyage_nano_field_sweep/sweep.py``) — this is
the identical 105-combo grid run against a different encoder: the
``queryonly_back_look_001`` LoRA adapter
(``baselines/twotower_queryonly_back_look_field_sweep/encode.py``), the new
best fine-tuned model in the project as of this experiment (see this
...
from baselines.twotower_queryonly_back_look_field_sweep.encode import (
    QUERYONLY_BACK_LOOK_ADAPTER_DIR,
    QUERYONLY_BACK_LOOK_BASE_MODEL,
    QUERYONLY_BACK_LOOK_MAX_SEQ_LENGTH,
    QUERYONLY_BACK_LOOK_TRUNCATE_DIM,
    QueryonlyBackLookEncoder,
    cosine_scores,
)
from baselines.twotower_queryonly_back_look_field_sweep.fields import Combo, all_combos, subset_id
from baselines.twotower_queryonly_back_look_field_sweep.text import candidate_to_text, seeker_to_text
...
def run_sweep(
    *,
    ...
    batch_size: int,
    max_length: int = QUERYONLY_BACK_LOOK_MAX_SEQ_LENGTH,
    ...
    encoder = QueryonlyBackLookEncoder(
