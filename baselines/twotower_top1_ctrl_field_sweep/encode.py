...
cell in "Which Lever Actually Worked?", `docs/twotower-ablation-verdict.md`).
Holdout: pair AUC 0.5974, hard-neg AUC 0.6121, MRR 0.5436 — the best of the
two `top1_optimised` runs (`top1_001` scores 0.5629 / 0.5241 / 0.4735).
...
wrapper that ``baselines/voyage_nano/encode.py::VoyageNanoEncoder`` has and
``encode_role`` doesn't, needed for the same encoding-dedup trick
``voyage_nano_field_sweep/sweep.py`` uses (22 unique encode groups instead
of 105 full passes).
"""
