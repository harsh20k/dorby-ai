...
Byte-identical duplicate of ``baselines/voyage_nano_field_sweep/text.py``
(same field-subset x query-flag parametrization), copied rather than
imported — this package re-runs that same 105-combo sweep design against a
different encoder (the fine-tuned ``top1_ctrl`` LoRA adapter instead of
frozen Voyage-4-nano), per the "ML / data-science experiments" isolation
rule in CLAUDE.md: a new encoder is a new experiment, not a new mode of an
existing one.
"""
