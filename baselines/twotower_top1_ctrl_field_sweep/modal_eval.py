"""Modal GPU entrypoint for the 105-combo top1_ctrl field/query sweep.

One remote call loads voyage-4-nano + the top1_ctrl LoRA adapter once and
runs all 105 combos in-process (sweep.run_sweep dedups encoding to 22
unique encode groups instead of 105 — see sweep.py's docstring), so this
needs one GPU boot, not 105.

Usage:
  modal run baselines/twotower_top1_ctrl_field_sweep/modal_eval.py
  modal volume get dorby-twotower-top1-ctrl-field-sweep-eval real_all ./artifacts/twotower_top1_ctrl_field_sweep_modal/real_all
"""
...
APP_NAME = "dorby-twotower-top1-ctrl-field-sweep"
RESULTS_VOLUME = "dorby-twotower-top1-ctrl-field-sweep-eval"
HF_CACHE_VOLUME = "dorby-twotower-hf-cache"  # reuse the existing HF model cache
ADAPTER_LOCAL_DIR = "artifacts/twotower_top1_optimised/top1_ctrl_001/adapter"
...
def sweep_remote(
    ...
    from baselines.twotower_top1_ctrl_field_sweep.sweep import run_sweep
...
def main(
    ...
    """CLI: modal run baselines/twotower_top1_ctrl_field_sweep/modal_eval.py"""
    call = sweep_remote
    ...
    print("=== Modal top1_ctrl field/query sweep finished ===")
    ...
    print(
        ...
        f"./artifacts/twotower_top1_ctrl_field_sweep_modal/{result['run_id']}"
    )
