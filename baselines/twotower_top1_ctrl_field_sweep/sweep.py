"""Run all 105 field/query combinations against the fine-tuned top1_ctrl checkpoint.

Same sweep design as ``baselines/voyage_nano_field_sweep/sweep.py`` (frozen
Voyage-4-nano) — this is the identical 105-combo grid run against a
different encoder: the ``top1_ctrl_001`` LoRA adapter
(``baselines/twotower_top1_ctrl_field_sweep/encode.py``), this project's
best fine-tuned model on the metric/population CLAUDE.md treats as
authoritative. Encoding dedup is unchanged: candidate-side embeddings
depend only on ``candidate_fields`` (7 unique groups); seeker-side
...
from baselines.twotower_top1_ctrl_field_sweep.fields import Combo, all_combos, subset_id
from baselines.twotower_top1_ctrl_field_sweep.text import candidate_to_text, seeker_to_text
...
def run_sweep(
    *,
    ...
    batch_size: int,
    max_length: int = TOP1_CTRL_MAX_SEQ_LENGTH,
