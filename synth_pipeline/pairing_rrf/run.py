...
from synth_pipeline.pairing_rrf import query_gen, recall, store
from synth_pipeline.pairing_rrf.judge import Judge
from synth_pipeline.pairing_rrf.sections import query_targets

DEFAULT_BEDROCK_MODEL = "google.gemma-3-27b-it"
...
class RunConfig:
    ...
    def batch_dir(self) -> Path:
        return Path(self.artifacts_dir) / "pairing_rrf" / self.batch_id
...
def run(cfg: RunConfig) -> dict[str, Any]:
    ...
    if cfg.skip_judge:
        print("--skip-judge set; stopping before any OpenRouter spend.")
    ...
    # ---- 7. judge -----------------------------------------------------------
    step = time.time()
    judge = Judge(cache_dir=out_dir / "judge_cache")
    items = []
