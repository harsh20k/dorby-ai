"""CLI for the RRF pairing pipeline.

    export AWS_PROFILE=tf_provisioner AWS_DEFAULT_REGION=us-east-1
    python -m synth_pipeline.pairing_rrf \
...
spending anything on OpenRouter — the shortlists are written either way, so this
...
from synth_pipeline.pairing_rrf.run import DEFAULT_BEDROCK_MODEL, RunConfig, run

REPO_ROOT = Path(__file__).resolve().parents[2]


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python -m synth_pipeline.pairing_rrf")
