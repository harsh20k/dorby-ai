...
- Try **Mixture of Experts** architecture — but note the natural expert decomposition here (one per intent) is currently **not viable**: the 69-pair holdout is 60 `fundraise` vs `hiring` 4 / `customers` 3 / `partnerships` 2, so four of five buckets are `low_n` and can't even produce an AUC. Revisit only if the intent distribution broadens. Real MoE does suit the <100 ms budget (sparse top-k activation keeps compute near dense cost, and it composes with two-tower by embedding candidates under each expert offline, then routing the query to expert *k* and searching only index *k* — at E× index storage). See the learned section-gate bullet below for the part of this idea that *is* actionable today.
...
- Possible experiment: **learned section-gate over the seeker's `lookingFor`** — the actionable, data-affordable version of the MoE idea above. Splitting the *seeker* into its `###` sections already works: pair AUC 0.579 → **0.596**, top-1 retrieval 27.6% → **34.5%** (candidate-sectioning, the original hypothesis, *lost* on every metric — see [docs/lookingfor-sectioning-findings.md](docs/lookingfor-sectioning-findings.md)). The reason is a routing problem: a seeker usually has several unrelated live threads (fundraising, hiring, investor intros) but any one `searchQuery` is about only one of them, so isolating the on-topic section stops the others from diluting the score. Aggregation over those sections is currently *fixed* — hard max, plus the softmax variants already tried — i.e. a gate with zero parameters. The increment is to make it **learned**: a small attention that reads the query and weights the seeker's sections accordingly ("this query is about fundraising → weight that section 0.8, the hiring one 0.05"). That captures MoE's useful idea (conditional routing) without MoE's expensive requirement (many parameter sets, each needing its own training data) — a few hundred parameters, which 131 real train pairs can plausibly support, where five intent experts cannot. Two hazards: with data this small the gate can collapse to a constant, which is just the fixed aggregation with extra steps, so check the learned weights actually vary by query; and do **not** train it on current pairing-batch labels — those are 0.868-AUC predictable from keyword overlap alone and the intent buckets are themselves keyword heuristics over `searchQuery`, so the gate would learn to route by keywords into experts trained on keyword-derived labels, counting one signal three times (the [docs/possible-bugs.md](docs/possible-bugs.md) #4 failure family in a new form). Also worth measuring against the dispersion finding: sectioning's benefit concentrated almost entirely in multi-intent seekers (MRR 0.194 → 0.273) versus focused ones (0.420 → 0.421), so a learned gate should help the same slice or the mechanism isn't what we think it is.
