...
by `tests/test_field_pos_look.py::test_preset_matches_top1_ctrl`.
...
`scripts/build_rrf_multineg_triplets_pos_look.py`, not by anything in
`TrainConfig`.
"""
...
def build_config(
    ...
    **overrides: Any,
    ...
    fields.setdefault("output_dir", Path("artifacts/twotower_field_pos_look") / run_id)
