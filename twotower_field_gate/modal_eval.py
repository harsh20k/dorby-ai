"""Modal GPU entrypoint: score the field-gate model on all 200 real pairs.
Seeker side = query + lookingFor + positioning pieces, combined by the
trained gate; candidate side = candidate_to_text — matching training
exactly. Uses ``SentenceTransformer.encode(..., truncate_dim=..., normalize_
embeddings=True)`` for every encode, so eval always uses the library's own
truncation path (no risk of the raw-forward/untruncated mismatch worked
around during training — see train.py's ``_encode`` docstring).
...
RESULTS_VOLUME = "dorby-twotower-field-gate-eval-results"
...
image = (
    ...
    .add_local_python_source(
        "twotower_field_gate", "query_weighted", "eval_real_full", "twotower", "baselines", "synth_pipeline"
    )
...
def eval_remote(run_id: str, batch_size: int = 8) -> dict:
    ...
    from twotower_field_gate.config import build_config, PIECE_KEYS
    ...
    model.load_adapter(str(ckpt_dir / "adapter"))

    gate = FieldGate(n_pieces=len(PIECE_KEYS), dim=cfg.truncate_dim).to("cuda")
    gate.load_state_dict(torch.load(ckpt_dir / "gate.pt", map_location="cuda"))
    gate.eval()

    def encode(texts, prompt):
        raw = model.encode(
            [prompt + t for t in texts], batch_size=batch_size, show_progress_bar=False,
            normalize_embeddings=True, convert_to_numpy=False, convert_to_tensor=True,
        )
        return raw

    @torch.no_grad()
    def encode_seeker(profiles_and_queries):
        piece_vecs = []
        for key in PIECE_KEYS:
            texts = [row[key] for row in profiles_and_queries]
            emb = torch.nn.functional.normalize(encode(texts, cfg.query_prompt), dim=-1)
            piece_vecs.append(emb)
        combined = gate(piece_vecs)
        return combined.cpu().numpy().astype(np.float32)

    @torch.no_grad()
    def encode_cand(texts):
        emb = torch.nn.functional.normalize(encode(texts, cfg.document_prompt), dim=-1)
        return emb.cpu().numpy().astype(np.float32)

    def seeker_pieces(r):
        profile = r["userContactFile"]
        return {
            "query": qtext.query_only(profile, r["searchQuery"]),
            "lookingFor": f"lookingFor: {(profile.get('lookingFor') or '').strip()}",
            "positioning": f"positioning: {(profile.get('positioning') or '').strip()}",
        }
    ...
    for subset in ("all", "train", "holdout"):
        ...
        pos_pieces = [seeker_pieces(r) for r in positives]
        neg_pieces = [seeker_pieces(r) for r in negatives]
        ...
        pos_seeker_emb = encode_seeker(pos_pieces)
        neg_seeker_emb = encode_seeker(neg_pieces)
        pos_cand_emb = encode_cand(pos_cand_texts)
        neg_cand_emb = encode_cand(neg_cand_texts)
        corpus_emb = encode_cand(corpus_texts)

        pos_seeker_emb, neg_seeker_emb = l2_normalize(pos_seeker_emb), l2_normalize(neg_seeker_emb)
        pos_cand_emb, neg_cand_emb, corpus_emb = l2_normalize(pos_cand_emb), l2_normalize(neg_cand_emb), l2_normalize(corpus_emb)
        ...
        neg_seeker_texts = [p["query"] for p in neg_pieces]  # hardness split reference text
...
def main(run_id: str = "field_gate_001", batch_size: int = 8) -> None:
    summary = eval_remote.remote(run_id, batch_size)
    print(f"\n=== {run_id} field-gate, all 200 real pairs ===")
    print(
    ...
    print(
        ...
        f"./artifacts/twotower_field_gate/{run_id}_real200"
    )
