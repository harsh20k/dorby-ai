...
Usage (all three arms keep train_batch_size * gradient_accumulation_steps = 12):
  # Arm A — batch only
  python -m twotower_qwen_bigbatch.train --run-id abl_a_batch_only \
      --rows-path artifacts/twotower_qwen_bigbatch/rrf_003_multineg_k1.json \
      --negatives-per-anchor 1 --train-batch-size 6 --gradient-accumulation-steps 2
  # Arm B — negatives only
  python -m twotower_qwen_bigbatch.train --run-id abl_b_negs_only \
      --rows-path artifacts/twotower_qwen_bigbatch/rrf_003_multineg_k2.json \
      --negatives-per-anchor 2 --train-batch-size 2 --gradient-accumulation-steps 6
  # Arm C — baseline corner
  python -m twotower_qwen_bigbatch.train --run-id abl_c_baseline \
      --rows-path artifacts/twotower_qwen_bigbatch/rrf_003_multineg_k1.json \
...
from twotower_qwen_bigbatch.checkpoint import select_best_checkpoint_with_dtype
...
from twotower_qwen_bigbatch.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_qwen_bigbatch.eval_dev import MultiNegTripletDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_qwen_bigbatch/modal_train.py)")
    ...
    # gradient checkpointing, or no micro-batch above 1 fits. twotower.train's
    ...
    meta.update(
        {
            "experiment": "twotower_qwen_bigbatch",
