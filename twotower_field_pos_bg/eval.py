...
trimmed to positioning+background, matching what the model was trained on.
...
def _trimmed_corpus(positives: list[dict], negatives: list[dict]) -> tuple[list[str], list[str]]:
    ...
    for record in positives + negatives:
        ...
        seen.add(mid)
        ids.append(mid)
        texts.append(pos_background(record["matchContactFile"]))
    return ids, texts


def evaluate_subset(model, positives: list[dict], negatives: list[dict], *, batch_size: int = 8) -> dict[str, Any]:
    pos_seeker = [pos_background(r["userContactFile"]) for r in positives]
    neg_seeker = [pos_background(r["userContactFile"]) for r in negatives]
    pos_cand = [pos_background(r["matchContactFile"]) for r in positives]
    neg_cand = [pos_background(r["matchContactFile"]) for r in negatives]
