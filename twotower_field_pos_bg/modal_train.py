"""Modal GPU entrypoint: LoRA fine-tune on positioning+background-only text.

Own app and own checkpoint volume — never touches any prior experiment's
volume. Mounts this package's own row file (built by
`scripts/build_rrf_multineg_triplets_pos_bg.py` from the same 643-row
...
`top1_ctrl`'s row file, since the text differs.
  ...
  modal volume get dorby-twotower-field-pos-bg-checkpoints <run_id> \\
      ./artifacts/twotower_field_pos_bg/<run_id>
...
APP_NAME = "dorby-twotower-field-pos-bg"
CHECKPOINT_VOLUME = "dorby-twotower-field-pos-bg-checkpoints"
...
image = (
    ...
    .add_local_python_source("twotower_field_pos_bg", "twotower", "baselines", "synth_pipeline")
    .add_local_dir("data", remote_path="/root/data")
    # This package's own row file (trimmed text), not top1_ctrl's.
    .add_local_dir("artifacts/twotower_field_pos_bg", remote_path="/root/rows")
)
...
def train_remote(
    run_id: str,
    rows_filename: str = "rrf_003_multineg_k1_pos_bg.json",
    ...
    from twotower_field_pos_bg.train import run_training
...
def main(
    ...
    print("=== Modal field_pos_bg train finished ===")
    for k, v in result.items():
        print(f"{k}: {v}")
    print(
        f"\nPull artifacts with:\n"
        f"  modal volume get {CHECKPOINT_VOLUME} {run_id} "
        f"./artifacts/twotower_field_pos_bg/{run_id}"
    )
