...
Uses `twotower_field_pos_bg.eval.run_eval` (this package's own trimmed-text
...
docstring for why the shared path can't be reused unchanged here.
    ...
    modal volume get dorby-twotower-field-pos-bg-eval-results field_pos_bg_001 \\
        ./artifacts/twotower_field_pos_bg/field_pos_bg_001_real200
"""
...
RESULTS_VOLUME = "dorby-twotower-field-pos-bg-eval-results"
...
image = (
    ...
    .add_local_python_source(
        "twotower_field_pos_bg", "field_pairs_sweep", "eval_real_full", "twotower", "baselines", "synth_pipeline"
    )
...
def eval_remote(run_id: str, batch_size: int = 8) -> dict:
    ...
    from twotower_field_pos_bg.eval import run_eval, write_metrics
...
def main(run_id: str = "field_pos_bg_001", batch_size: int = 8) -> None:
    summary = eval_remote.remote(run_id, batch_size)
    print(f"\n=== {run_id}, all 200 real pairs (positioning+background, both sides) ===")
    ...
    print(
        ...
        f"./artifacts/twotower_field_pos_bg/{run_id}_real200"
    )
