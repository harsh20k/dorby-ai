...
trimmed to background+lookingFor, matching what the model was trained on.
...
def evaluate_subset(model, positives: list[dict], negatives: list[dict], *, batch_size: int = 8) -> dict[str, Any]:
    pos_seeker = [background_lookingfor(r["userContactFile"]) for r in positives]
    neg_seeker = [background_lookingfor(r["userContactFile"]) for r in negatives]
