"""LoRA fine-tune, `top1_ctrl`'s exact recipe, on background+lookingFor-only
...
Usage:
  python -m twotower_field_bg_look.train --run-id field_bg_look_001 \\
      --rows-path artifacts/twotower_field_bg_look/rrf_003_multineg_k1_bg_look.json \\
      --dry-run
"""
...
from twotower_field_bg_look.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_field_bg_look.eval_dev import CorpusRecallDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_field_bg_look/modal_train.py)")
    ...
    meta.update(
        {
            "experiment": "twotower_field_bg_look",
    ...
    # text, not the trimmed background+lookingFor this model was trained on
    ...
    # `twotower_field_bg_look.eval.run_eval` instead (all 200, trimmed both
