"""LoRA fine-tune, `top1_ctrl`'s exact recipe, on queryonly_back_look-only
...
Usage:
  python -m twotower_queryonly_back_look.train --run-id queryonly_back_look_001 \\
      --rows-path artifacts/twotower_queryonly_back_look/rrf_003_multineg_k1_queryonly_back_look.json \\
      --dry-run
"""
...
from twotower_queryonly_back_look.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_queryonly_back_look.eval_dev import CorpusRecallDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_queryonly_back_look/modal_train.py)")
    ...
    meta.update(
        {
            "experiment": "twotower_queryonly_back_look",
            "seeker_fields": [],
    ...
    # text, not the trimmed queryonly_back_look this model was trained on
    ...
    # `twotower_queryonly_back_look.eval.run_eval` instead (all 200, trimmed both
