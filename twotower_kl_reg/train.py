...
`top1_ctrl`'s exact recipe (`twotower_top1_optimised/train.py`, run-id
`top1_ctrl_001`) plus one new loss term — see `losses.py` and this package's
`__init__.py` for the full rationale. Reuses generic, model-agnostic helpers
from `twotower.train`/`twotower.eval` read-only; copies (does not import)
`twotower_top1_optimised`'s `data.py`/`eval_dev.py`, per the isolation rule.

Usage:
  python -m twotower_kl_reg.train --run-id kl_reg_ctrl \\
      --rows-path artifacts/twotower_rrf_triplet_ablation/rrf_003_multineg_k1.json \\
      --kl-weight 0.5 --dry-run
"""
...
from twotower_kl_reg.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_kl_reg.eval_dev import CorpusRecallDevEvaluator
from twotower_kl_reg.losses import KLRegularizedMNRL
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_kl_reg/modal_train.py)")
