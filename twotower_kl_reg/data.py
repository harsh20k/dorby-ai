...
Verbatim copy of ``twotower_top1_optimised/data.py`` (pinned by
``tests/test_kl_reg.py::test_data_py_matches_top1_optimised``), not an
import — this experiment trains on exactly the same rows `top1_ctrl` did, so
the copy must never silently drift from the original.
"""
