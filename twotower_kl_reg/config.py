"""Config for the KL-regularized arm: `top1_ctrl`'s exact recipe plus one new
loss term, nothing else varied.

`MODEL_PRESETS` below reproduces `top1_ctrl_001`'s settings field-for-field
(`twotower_top1_optimised/config.py`'s preset with `loss_scale=20.0`,
`hardness_mode=None` — the control corner, not the sharpened `top1_001` arm):
rank 8 / alpha 16 / dropout 0.05 on q/k/v/o_proj, micro-batch 6, accum 2
(effective batch 12), lr 2e-4, `primary_metric="recall@1"`. Pinned by
`tests/test_kl_reg.py::test_preset_matches_top1_ctrl`.

The only new knob is `kl_weight` — how strongly the adapted model is
penalized for its in-batch similarity distribution diverging from the frozen
base model's. LoRA's B matrix is zero-initialized, so adapted == frozen at
step 0 and the penalty is exactly 0 at the start of training — it only grows
as the adapter moves away from the frozen model, so no warmup schedule is
needed. 0.5 is a first, reasoned-but-untuned choice (main loss and KL are
both O(1) at this scale/batch size); a follow-up sweep is a natural next step
if this weight moves nothing.
"""
...
MODEL_PRESETS: dict[str, dict[str, Any]] = {
    "voyage-4-nano": {
        ...
        # top1_ctrl's winning corner, held fixed — see module docstring.
        "train_batch_size": 6,
        "eval_batch_size": 6,
...
# `extra` and are recorded in run_meta.json for every run. scale/hardness are
# pinned to top1_ctrl's exact values (library defaults); only kl_weight is new.
...
def build_config(
    ...
    **overrides: Any,
    ...
    fields.setdefault("output_dir", Path("artifacts/twotower_kl_reg") / run_id)
