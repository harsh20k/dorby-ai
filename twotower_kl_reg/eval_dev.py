...
``tests/test_kl_reg.py``, import path updated only) — reused unchanged so
checkpoint selection works identically to `top1_ctrl`'s and any difference in
final results is attributable to the new KL term, not a different selection
rule.
...
from twotower_kl_reg.data import MultiNegRow
