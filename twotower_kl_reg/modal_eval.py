"""Modal GPU entrypoint: score the KL-regularized adapter on all 200 real pairs.

Seeker/candidate text is unchanged from `top1_ctrl` (profile + query
concatenated / candidate profile only) — this experiment varies only the loss,
not the input representation — so scoring reuses `eval_real_full.eval.run_eval`
directly, read-only, exactly the same function `top1_ctrl`'s own all-200 number
in `docs/baseline-results-real200.md` was computed with. No new scoring logic
needed (unlike `twotower_split/` or `twotower_field_gate/`, which changed the
input shape and had to hand-roll their own).

    modal run twotower_kl_reg/modal_eval.py --run-id kl_reg_ctrl
    modal volume get dorby-twotower-kl-reg-eval-results kl_reg_ctrl \\
        ./artifacts/twotower_kl_reg/kl_reg_ctrl_real200
"""
...
RESULTS_VOLUME = "dorby-twotower-kl-reg-eval-results"
...
image = (
    ...
    .add_local_python_source("twotower_kl_reg", "eval_real_full", "twotower", "baselines", "synth_pipeline")
...
def main(run_id: str = "kl_reg_ctrl", batch_size: int = 8) -> None:
    summary = eval_remote.remote(run_id, batch_size)
    print(f"\n=== {run_id}, all 200 real pairs ===")
    ...
    print(
        ...
        f"./artifacts/twotower_kl_reg/{run_id}_real200"
    )
