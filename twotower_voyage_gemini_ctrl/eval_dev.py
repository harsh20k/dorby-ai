...
Why this replaces the ablation's evaluator
------------------------------------------
`twotower_rrf_triplet_ablation/eval_dev.py` reports `beat_all_accuracy`: for each
row, did the positive out-score *its own* one or two negatives. That is a triplet
test over 2-3 candidates. The metric everything is finally judged on ranks the
positive against a corpus of ~178. Those are different tasks, and the gap is not
academic — it is why "best epoch" has repeatedly failed to predict holdout
retrieval, and why fixing checkpoint selection in the ablation made the holdout
numbers *worse*: the selector was optimising the wrong thing, so its choice was
close to arbitrary.

This evaluator ranks each dev anchor against **every unique candidate in the dev
split** and reports recall@1/@5/@10 and MRR. `primary_metric` defaults to
`recall@1`, so `select_best_checkpoint` finally optimises the quantity this
experiment exists to improve.

Scoring is delegated to `baselines.metrics.retrieval_metrics` **unchanged** —
the same function `twotower.eval.evaluate_pairs` calls for the real holdout. Dev
and holdout therefore compute recall@1 identically, by construction rather than
by convention.

Caveat kept deliberately: when one seeker has several positives, ranking a
*different* positive of the same seeker first counts as a miss. That matches how
`retrieval_metrics` treats the holdout (one target id per query), so the dev
number stays comparable rather than being made flattering.
"""
...
from twotower_voyage_gemini_ctrl.data import MultiNegRow
