"""LoRA fine-tune with MultipleNegativesRankingLoss — top1_ctrl's exact recipe,
retrained on the pairing_voyage_gemini batch instead of rrf_003.

Separate experiment from twotower_top1_optimised/ — see this package's
__init__.py for the full rationale, the leakage caveat found on the new batch
before training, and why eval_real_full/guard.py and modal_eval.py each gained
one additive entry to score this arm. Reuses generic, model-agnostic helpers
from twotower.train/twotower.eval read-only; imports nothing from
twotower_top1_optimised (a prior run's frozen code).

Usage (effective batch stays 12, matching top1_ctrl_001):
  python -m twotower_voyage_gemini_ctrl.train --run-id voyage_gemini_ctrl_001 \
...
from twotower_voyage_gemini_ctrl.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_voyage_gemini_ctrl.eval_dev import CorpusRecallDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_voyage_gemini_ctrl/modal_train.py)")
    ...
    meta.update(
        {
            "experiment": "twotower_voyage_gemini_ctrl",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    # defaults = top1_ctrl_001's exact recipe, effective batch 12
