"""Rescore ask_offer_001 on all 200 real pairs with offer = positioning + background.

Eval-only. Does not retrain. Does not score the 69-pair holdout separately.
twotower_ask_offer/ is not edited.
"""
...
from twotower_ask_offer_posbg_eval.text import bg_text, look_text, seeker_look_text

# Published ask_offer_001 all-200 numbers (wide offer text). Quoted, not recomputed.
ORIGINAL_ALL200 = {
    "pair_auc_forward_only": 0.5126,
    "pair_auc_combined": 0.5714,
    "mrr_forward_only": 0.4317,
    "recall@1_forward_only": 0.2700,
    "offer_text": "all profile fields except lookingFor",
}
...
def evaluate_ask_offer_pairs(
    ...
    *,
    ...
    seeker_ask_texts = [
        seeker_look_text(lp.pair["userContactFile"], lp.pair["searchQuery"]) for lp in pairs
    ]
    ...
    k_seek = encode_batched(
        ask_model, seeker_ask_texts, role="query", cfg=cfg, device=device, batch_size=batch_size
    )
    v_seek = encode_batched(
        offer_model, seeker_offer_texts, role="document", cfg=cfg, device=device, batch_size=batch_size
    )
    k_cand = encode_batched(
        ask_model, cand_ask_texts, role="query", cfg=cfg, device=device, batch_size=batch_size
    )
    v_cand = encode_batched(
        offer_model, cand_offer_texts, role="document", cfg=cfg, device=device, batch_size=batch_size
    )
    ...
    cand_corpus_emb = encode_batched(
        offer_model, cand_bg_texts, role="document", cfg=cfg, device=device, batch_size=batch_size
    )
    ...
    neg_seeker_texts = [
        seeker_to_text(p["userContactFile"], p["searchQuery"]) for p in negatives
    ]
...
def run_eval(
    ...
    *,
    ...
    print(
        ...
        f"{real_all.n_candidates} candidates — all 200 real pairs"
    )
    if len(pairs) != 200:
        raise AssertionError(f"expected 200 real pairs, got {len(pairs)}")

    scored = evaluate_ask_offer_pairs(
        ask_model,
        offer_model,
        pairs,
        lam=lam,
    ...
    return {
        ...
        "lam": lam,
        "population": "all-200 real pairs",
        "ask_fields": ["lookingFor", "searchQuery (seeker only)"],
        "offer_fields": ["positioning", "background"],
        "note": (
            "Eval-time field swap on ask_offer_001 adapters. Offer tower was "
            "trained on all fields except lookingFor; this run feeds "
            "positioning+background only. Lambda is the training value 1.75, "
            "not refit."
        ),
        "original_ask_offer_001_all200": ORIGINAL_ALL200,
        "all200": scored,
    }
...
def print_summary(metrics: dict[str, Any]) -> None:
    ...
    s = metrics["all200"]
    print(
        f"\n=== all-200 ({s['num_positive']} pos / {s['num_negative']} neg, "
        f"{s['num_candidates']} candidates) ==="
    )
    ...
    if hn.get("easy") and hn.get("hard"):
        print(
            f"neg-hardness AUC (combined): easy={hn['easy'].get('pair_auc')} "
            f"hard={hn['hard'].get('pair_auc')}"
        )
    orig = metrics["original_ask_offer_001_all200"]
    print("\n=== vs original ask_offer_001 all-200 (wide offer text) ===")
    print(
        f"combined AUC  this={s['pair_combined']['roc_auc']:.4f}  "
        f"original={orig['pair_auc_combined']:.4f}"
    )
    print(
        f"forward AUC   this={s['pair_forward_only']['roc_auc']:.4f}  "
        f"original={orig['pair_auc_forward_only']:.4f}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = args.out
    if out is None:
        out = Path("artifacts/twotower_ask_offer_posbg_eval/metrics.json")
