...
Copy of ``twotower_kl_reg/losses.py`` (pinned by
``tests/test_voyage_gemini_kl.py``), not an import — the isolation rule
requires the KL mechanism be copied rather than reimplemented so this arm's
loss is provably the same one `kl_reg_ctrl_001` used, just applied to
`voyage_gemini_ctrl_001`'s bigger training population instead of `rrf_003`.
...
candidates instead of a token distribution.
