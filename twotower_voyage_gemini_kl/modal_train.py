"""Modal GPU entrypoint for the KL-regularized arm on the pairing_voyage_gemini batch.

Own app and own checkpoint volume — never touches `voyage_gemini_ctrl`'s,
`kl_reg`'s, or any other prior experiment's volume. Shares only the HF
model-download cache (keyed by model name) and reads (never writes) the same
row file `voyage_gemini_ctrl_001` trained on, mounted read-only from
`twotower_voyage_gemini_ctrl`'s artifacts dir.

  modal run --detach twotower_voyage_gemini_kl/modal_train.py --run-id voyage_gemini_kl_001

  modal app logs <app-id>   # fetch mode, NOT -f — local_entrypoint() print
                             # statements (final summary, holdout metrics, pull
                             # command) do NOT show up here; redirect the
                             # launching shell's stdout/stderr to a log file
                             # instead and read that.
  modal volume get dorby-twotower-voyage-gemini-kl-checkpoints <run_id> \\
      ./artifacts/twotower_voyage_gemini_kl/<run_id>
...
image = (
    ...
    .add_local_python_source(
        "twotower_voyage_gemini_kl", "twotower", "baselines", "synth_pipeline"
    )
    .add_local_dir("data", remote_path="/root/data")
    # Reuses voyage_gemini_ctrl's row file read-only, so this arm trains on
    # exactly the population voyage_gemini_ctrl_001 did.
...
def train_remote(
    ...
    from twotower_voyage_gemini_kl.train import run_training
...
def main(
    run_id: str = "voyage_gemini_kl_001",
    rows_filename: str = "voyage_gemini_smoke002_multineg_k1.json",
    negatives_per_anchor: int = 1,
    epochs: int = 5,
    seed: int = 42,
    ...
    if eff != 12:
        print(
            ...
            "!= 12 — not comparable to voyage_gemini_ctrl_001."
        )
    ...
    print("=== Modal voyage_gemini_kl train finished ===")
    for k, v in result.items():
        print(f"{k}: {v}")
    print(
        f"\nPull artifacts with:\n"
        f"  modal volume get {CHECKPOINT_VOLUME} {run_id} "
        f"./artifacts/twotower_voyage_gemini_kl/{run_id}"
    )
