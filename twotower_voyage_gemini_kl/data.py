...
Copy of ``twotower_voyage_gemini_ctrl/data.py`` (pinned by
``tests/test_voyage_gemini_kl.py::test_data_py_matches_voyage_gemini_ctrl``),
not an import — this experiment trains on exactly the same
`pairing_voyage_gemini/smoke_test_002`-derived rows `voyage_gemini_ctrl_001`
did, so the copy must never silently drift from the original. These rows come
from synthetic profiles labeled by an LLM judge, never touch a real user, so
there is no holdout-leak surface against the real seed_split; the real
69-pair holdout is only ever touched by `twotower.eval.evaluate_pairs` at the
end of training.
"""
