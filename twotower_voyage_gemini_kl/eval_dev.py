...
Copy of ``twotower_voyage_gemini_ctrl/eval_dev.py`` (pinned by
``tests/test_voyage_gemini_kl.py``, import path updated only) — reused
unchanged so checkpoint selection works identically to
`voyage_gemini_ctrl_001`'s and any difference in final results is
attributable to the new KL term, not a different selection rule.
...
holdout. Dev and holdout therefore compute recall@1 identically, by
construction rather than by convention.

Caveat kept deliberately: when one seeker has several positives, ranking a
*different* positive of the same seeker first counts as a miss. That matches
how `retrieval_metrics` treats the holdout (one target id per query), so the
dev number stays comparable rather than being made flattering.
"""
...
from twotower_voyage_gemini_kl.data import MultiNegRow
