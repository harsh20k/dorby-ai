"""Prompt + field-trimmed text serialization for the focused LLM-judge variant.

FROZEN COPY — provenance
------------------------
Vendored verbatim on 2026-08-08 from::

    baselines/llm_judge_with_pos_look_pos_back_look/prompt.py
    sha256 2c80b24cbf463d997c61ca2b98a50042489df68bf6467d2bee1cc6b054e7292d

which lives on the ``llm-judge-experiment`` branch (worktree
``.claude/worktrees/llm-judge-experiment``), not on ``main``. Copied rather
than imported for two reasons: that path is not importable from ``main``, and
the experiment-isolation rule in CLAUDE.md requires an experiment to own a
frozen copy of its inputs. ``tests/test_judge_prompt_evolution_focused.py``
pins this copy against the source hash whenever that worktree is present.

Original module docstring follows.

Deliberately different from ``baselines/llm_judge/prompt.py`` in two ways:

* the seeker's ``searchQuery`` is included (the original experiment's
  whole point was withholding it);
* only a subset of profile fields is shown, not the complete profile
  (seeker: ``positioning`` + ``lookingFor``; candidate: ``positioning`` +
  ``background`` + ``lookingFor``), rather than every field from
  ``baselines/bert_frozen/text.py::PROFILE_FIELDS``.

Pushed to LangSmith Prompt Hub as ``llm_judge_with_pos_look_pos_back_look``
(see ``scripts/push_llm_judge_focused_prompt.py``); the string here is the
source of truth pushed from, not pulled at eval time — eval always runs the
exact text below, matching how ``baselines/llm_judge/prompt.py`` keeps its
prompt inline rather than fetched from the hub per call.
"""
...
def seeker_to_text(user_contact_file: Mapping[str, Any]) -> str:
    """Seeker text = positioning + lookingFor only, no searchQuery (added separately)."""
...
PROMPT_NAME = "llm_judge_with_pos_look_pos_back_look"
