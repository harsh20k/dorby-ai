...
from judge_prompt_evolution_focused.hub import (
    push_iteration_prompt,
    push_meta_prompt,
    push_seed_prompt,
    push_summarizer_prompt,
)
from judge_prompt_evolution_focused.optimizer import run_one_iteration, run_summarization_step
from judge_prompt_evolution_focused.sampling import ExampleBank
...
def resolve_seed_prompt(cfg: RunConfig) -> tuple[str, str]:
    ...
    Only ``focused`` is supported here — this package exists to evolve the
    focused prompt specifically. The ``naive`` / ``structured_cot`` seeds live
    in ``judge_prompt_evolution/`` (evo_001-evo_009) and are deliberately not
    re-offered, since this package's example rendering shows the trimmed
    focused field set and would mismatch either of those prompts.
    """
    if cfg.seed_source == "focused":
        return (
            SEED_JUDGE_PROMPT,
            "seed: unmodified focused judge prompt — searchQuery given, fields trimmed "
            "(seeker positioning+lookingFor, candidate positioning+background+lookingFor); "
            "scored 0.6451 pair AUC on all-200 real pairs / 0.6530 on the 69-pair holdout "
            "via the direct Google API (docs/llm-judge-focused-prompt-experiment.md)",
        )
...
def run(cfg: RunConfig, *, resume: bool = False) -> dict[str, Any]:
    ...
    summary = {
        ...
        "example_pool_note": (
            "Examples were sampled from split=" + repr(cfg.split) + "."
            + (
                " This includes the 69-pair holdout, chosen deliberately: the evolved prompt's"
                " intended downstream job is labeling new synthetic pairs, not scoring these 200."
                " The end-of-run AUC check is therefore measured on pairs the optimizer saw —"
                " read it as 'did the loop improve the prompt', not as a held-out"
                " generalization claim."
                if cfg.split != "train" else ""
            )
        ),
        ...
        "seed_prompt_auc_reference": {
            ...
            "all_200_pair_auc": 0.6451,
            "holdout_69_pair_auc": 0.6530,
            "backend": "direct Google API (gemini-3.1-flash-lite)",
            "source": "docs/llm-judge-focused-prompt-experiment.md",
        },
        "contract_warnings": [
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--seed-source", choices=["focused"], default=None)
