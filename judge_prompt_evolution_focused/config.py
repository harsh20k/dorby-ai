...
DEFAULT_ARTIFACTS_DIR = REPO_ROOT / "artifacts" / "judge_prompt_evolution_focused"

DEFAULT_OPTIMIZER_MODEL = "gemini-3.1-flash-lite"
DEFAULT_BASE_URL = "https://openrouter.ai/api/v1"

# Examples are sampled from all 200 real pairs, deliberately. The evolved
# prompt's intended job is labeling *new* synthetic pairs downstream, not
# scoring these 200 — so the all-200 pool buys a richer, more diverse example
# set at no cost to that use. The consequence for the end-of-run AUC check is
# recorded honestly: it is measured on pairs the optimizer saw, so it reads as
# "did the loop improve the prompt", not as a held-out generalization claim.
DEFAULT_SPLIT = "all"
...
class RunConfig:
    ...
    seed_source: str = "focused"

    artifacts_dir: Path = DEFAULT_ARTIFACTS_DIR

    push_to_hub: bool = True
    hub_repo: str = "judge-prompt-evolution-focused"
