"""LoRA fine-tune with MultipleNegativesRankingLoss — ablation arms isolating
micro-batch size from negatives-per-row, at fixed effective batch 12.

Separate experiment from twotower/'s Arm A/B/C, twotower_rrf_triplet/, and
twotower_rrf_triplet_bigbatch/ — see
docs/twotower-rrf-triplet-ablation-experiment.md for the 2x2 design, and for
the caveat that rrf_003's labels are an LLM judge's opinion on synthetic
profiles, not real accept/decline outcomes. Reuses generic, model-agnostic
helpers from twotower.train/twotower.eval read-only; imports nothing from the
two prior triplet packages (each is a prior run's frozen code).

Usage (all three arms keep train_batch_size * gradient_accumulation_steps = 12):
  # Arm A — batch only
  python -m twotower_top1_optimised.train --run-id abl_a_batch_only \
      --rows-path artifacts/twotower_top1_optimised/rrf_003_multineg_k1.json \
      --negatives-per-anchor 1 --train-batch-size 6 --gradient-accumulation-steps 2
  # Arm B — negatives only
  python -m twotower_top1_optimised.train --run-id abl_b_negs_only \
      --rows-path artifacts/twotower_top1_optimised/rrf_003_multineg_k2.json \
      --negatives-per-anchor 2 --train-batch-size 2 --gradient-accumulation-steps 6
  # Arm C — baseline corner
  python -m twotower_top1_optimised.train --run-id abl_c_baseline \
      --rows-path artifacts/twotower_top1_optimised/rrf_003_multineg_k1.json \
...
from twotower_top1_optimised.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_top1_optimised.eval_dev import CorpusRecallDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_top1_optimised/modal_train.py)")
    ...
    # Change 1: sharpen the loss toward the top competitor. Every prior run used
    # library defaults (scale=20.0, hardness_mode=None, hardness_strength=0.0);
    # the temperature was never tuned and hard-negative weighting never enabled.
    loss_kwargs: dict = {"scale": float(cfg.extra["loss_scale"])}
    hardness_mode = cfg.extra.get("hardness_mode")
    if hardness_mode:
        loss_kwargs["hardness_mode"] = hardness_mode
        loss_kwargs["hardness_strength"] = float(cfg.extra.get("hardness_strength", 0.0))
    try:
        loss = MultipleNegativesRankingLoss(model=model, **loss_kwargs)
    except TypeError as exc:
        # Older sentence-transformers lacks hardness_mode/hardness_strength.
        # Fail loudly rather than silently training a different experiment.
        raise RuntimeError(
            f"MultipleNegativesRankingLoss rejected {loss_kwargs!r} ({exc}). This "
            f"arm's whole premise is those arguments; upgrade sentence-transformers "
            f"rather than falling back to defaults."
        ) from exc
    print(f"loss: MultipleNegativesRankingLoss({loss_kwargs})")

    # Change 2: dev now ranks against the full dev corpus, so checkpoint
    # selection can see recall@1 instead of triplet accuracy.
    ...
    meta.update(
        {
            "experiment": "twotower_top1_optimised",
    ...
    # CorpusRecallDevEvaluator writes "train_dev_recall@1"; cfg.primary_metric is
    # already "recall@1" from this package's preset, so no override is needed —
    # but keep the explicit replace so a caller overriding primary_metric cannot
    # silently reintroduce the fall-back-to-final-epoch failure
    # (docs/possible-bugs.md #2, hit three times in this project).
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    # defaults = the baseline corner (Arm C); every other arm overrides what it
    # varies, keeping train_batch_size * gradient_accumulation_steps == 12
