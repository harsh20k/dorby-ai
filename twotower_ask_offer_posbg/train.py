...
from twotower_ask_offer_posbg.data import AskOfferRow, carve_dev, load_rows
from twotower_ask_offer_posbg.eval_dev import evaluate_dev
from twotower_ask_offer_posbg.loss import build_batch_texts, combine_and_cross_entropy
from twotower_ask_offer_posbg.model import build_two_towers, encode_texts, trainable_parameters
...
def run_training(
    ...
    *,
    ...
    meta.update(
        {
            "experiment": "twotower_ask_offer_posbg",
            "run_id": cfg.run_id,
            "config": cfg.to_dict(),
    ...
    if run_holdout and not dry_run:
        from twotower_ask_offer_posbg.eval import evaluate_ask_offer_pairs
...
def seeker_look_text_sample(row: AskOfferRow) -> str:
    from twotower_ask_offer_posbg.text import seeker_look_text
...
def parse_args(argv: list[str] | None = None):
    ...
    p.add_argument("--rows-path", type=Path, default=Path("artifacts/twotower_ask_offer_posbg/ask_offer_rows.json"))
