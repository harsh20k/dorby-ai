...
seeker and candidate — trimmed to positioning + lookingFor only, no query,
no other field. Feeds `twotower_field_pos_look/`.

Deliberate near-copy of scripts/build_rrf_multineg_triplets_pos_bg.py (its
sibling for the positioning+background pair, itself copied from
scripts/build_rrf_multineg_triplets.py), never imported from it, per the
repo's experiment-isolation rule. Only the field-pair text builder differs.

`field_pairs_sweep.text.pos_lookingfor` is a plain profile->text function
with no seeker/candidate-specific behavior, so it is reused unmodified for
both sides here.

Usage:
  python scripts/build_rrf_multineg_triplets_pos_look.py \\
      --batch-dir exports/rrf_datasets/rrf_003 --negatives-per-anchor 1 \\
      --out artifacts/twotower_field_pos_look/rrf_003_multineg_k1_pos_look.json
"""
...
def build_multineg_rows(
    ...
    for query_key, groups in sorted(by_query.items()):
        ...
        anchor_text = pos_lookingfor(anchor_pair["userContactFile"])
        if not anchor_text:
            n_empty_anchor += 1
        ...
        for pos_rec in pos_recs:
            ...
            pos_text = pos_lookingfor(pos_pair["matchContactFile"])
            if not pos_text:
                n_empty_candidate += 1
            ...
            neg_texts = [pos_lookingfor(p["matchContactFile"]) for p in neg_pairs]
            for t in neg_texts:
                if not t:
                    n_empty_candidate += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = (
        args.out
        or Path("artifacts/twotower_field_pos_look")
        / f"{args.batch_dir.name}_multineg_k{args.negatives_per_anchor}_pos_look.json"
    )
