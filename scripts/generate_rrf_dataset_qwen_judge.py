#!/usr/bin/env python3
"""Isolated copy of scripts/generate_rrf_dataset.py using the Qwen3-32B-on-Bedrock
judge (synth_pipeline.pairing_rrf_qwen_judge) instead of gemini-3.1-flash-lite.

Duplicated rather than adding a --judge-backend flag to the original per this
repo's experiment-isolation rule: the original script's batches (rrf_002, etc.)
must stay reproducible from unmodified code, and a judge-model swap changes what
any batch means, not just how it's produced. See
synth_pipeline/pairing_rrf_qwen_judge/__init__.py for the judge tradeoff
(pair AUC 0.5802 vs 0.6358, hard-neg AUC 0.6224 vs 0.6466, ~3x cheaper).

    python scripts/generate_rrf_dataset_qwen_judge.py --preset my_run.json \
...
DEFAULT_PRESET = REPO_ROOT / "synth_pipeline" / "pairing_rrf_qwen_judge" / "presets" / "default.json"
...
def run_pairing(preset: dict[str, Any], profile_run: Path, batch_id: str) -> dict[str, Any]:
    ...
    from synth_pipeline.pairing_rrf_qwen_judge.run import RunConfig, run as run_pipeline
    ...
    cfg = RunConfig(
        ...
        query_gen_concurrency=preset.get("query_gen_concurrency", 1),
    )
...
def export(
    ...
    batch_dir = REPO_ROOT / "artifacts" / "pairing_rrf_qwen_judge" / batch_id
...
def _readme(batch_id: str, profile_run: Path, summary: dict[str, Any]) -> str:
    ...
    return f"""# Synthetic pair batch `{batch_id}` (Qwen3-32B-on-Bedrock judge)

Produced by `scripts/generate_rrf_dataset_qwen_judge.py`. The exact settings
are in `preset_used.json` — re-running with that file reproduces this batch.
...
- Judge cost: ${cost.get('measured_judge_cost_usd', 0):.4f} (measured, from Bedrock token usage x published on-demand price)

## What these labels are

A model's opinion, not real accept/decline outcomes. The judge here is
`qwen.qwen3-32b-v1:0` on AWS Bedrock (naive framing, same prompt as the
flash-lite pipeline) — ~3x cheaper per call than the project's best-measured
judge (`google/gemini-3.1-flash-lite`, 0.6358 pair AUC / 0.5942 decision
accuracy), at Qwen3-32B's own measured 0.5802 pair AUC / 0.6224 hard-negative
AUC on the real 69-pair holdout (docs/llm-judge-experiment.md). There is no
deadband: every judged pair is labeled, so borderline pairs carry some wrong
labels — and more of them than the flash-lite batches, given the lower AUC.
...
def main(argv: list[str] | None = None) -> int:
    ...
    logs = REPO_ROOT / "artifacts" / "pairing_rrf_qwen_judge" / batch_id
