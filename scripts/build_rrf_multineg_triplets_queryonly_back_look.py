"""Extract (anchor, positive, negative_1..negative_k) rows: seeker = search
query ONLY (no profile fields at all), candidate = background + lookingFor.
Feeds `twotower_queryonly_back_look/`.

This is the winning combo found by the top1_ctrl 105-way field/query sweep
(seeker field-count 0, candidate=background+lookingFor, query included) —
best recall@1 (0.28) of any combo tested, scored frozen against the
existing top1_ctrl checkpoint. Nobody has trained on this combo yet; this
script builds the row file to do that.

Deliberate near-copy of scripts/build_rrf_multineg_triplets_pos_bg.py
(itself copied from scripts/build_rrf_multineg_triplets.py), never
imported from it, per the repo's experiment-isolation rule. Reuses two
existing, unmodified builders read-only:
``query_weighted.text.query_only`` for the seeker side (falls back to the
profile only if a pair's query is empty — verified 0/643 in this batch) and
``field_pairs_sweep.text.background_lookingfor`` for the candidate side.

Usage:
  python scripts/build_rrf_multineg_triplets_queryonly_back_look.py \\
      --batch-dir exports/rrf_datasets/rrf_003 --negatives-per-anchor 1 \\
      --out artifacts/twotower_queryonly_back_look/rrf_003_multineg_k1_queryonly_back_look.json
"""
...
def build_multineg_rows(
    ...
    for query_key, groups in sorted(by_query.items()):
        ...
        if not (anchor_pair["searchQuery"] or "").strip():
            n_empty_query += 1
        ...
        for pos_rec in pos_recs:
            ...
            pos_text = background_lookingfor(pos_pair["matchContactFile"])
            if not pos_text:
                n_empty_candidate += 1
            ...
            neg_texts = [background_lookingfor(p["matchContactFile"]) for p in neg_pairs]
            for t in neg_texts:
                if not t:
                    n_empty_candidate += 1
    ...
    summary = {
        ...
        "seeker_fields": [],  # query only, no profile fields
        ...
        "n_empty_query_fallback": n_empty_query,
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = (
        args.out
        or Path("artifacts/twotower_queryonly_back_look")
        / f"{args.batch_dir.name}_multineg_k{args.negatives_per_anchor}_queryonly_back_look.json"
    )
    ...
    print(f"seeker: query only  candidate_fields: {summary['candidate_fields']}")
    print(f"rows: {summary['n_rows']}")
    print(f"distinct seekers: {summary['n_seekers']}")
    print(f"empty query fallback: {summary['n_empty_query_fallback']}  empty candidate text: {summary['n_empty_candidate_text']}")
