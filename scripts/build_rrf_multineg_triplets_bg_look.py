...
seeker and candidate — trimmed to background + lookingFor only, no query,
no other field. Feeds `twotower_field_bg_look/`.

Deliberate near-copy of scripts/build_rrf_multineg_triplets_pos_bg.py (its
sibling for the positioning+background pair), never imported from it, per
the repo's experiment-isolation rule. Only the field-pair text builder
differs.

`field_pairs_sweep.text.background_lookingfor` is a plain profile->text
function with no seeker/candidate-specific behavior, so it is reused
unmodified for both sides here.

Usage:
  python scripts/build_rrf_multineg_triplets_bg_look.py \\
      --batch-dir exports/rrf_datasets/rrf_003 --negatives-per-anchor 1 \\
      --out artifacts/twotower_field_bg_look/rrf_003_multineg_k1_bg_look.json
"""
...
def build_multineg_rows(
    ...
    for query_key, groups in sorted(by_query.items()):
        ...
        anchor_text = background_lookingfor(anchor_pair["userContactFile"])
        if not anchor_text:
            n_empty_anchor += 1
    ...
    summary = {
        ...
        "seeker_fields": ["background", "lookingFor"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = (
        args.out
        or Path("artifacts/twotower_field_bg_look")
        / f"{args.batch_dir.name}_multineg_k{args.negatives_per_anchor}_bg_look.json"
    )
