"""Unit tests for baselines/reciprocal_lambda_grid_voyage_gemini_ctrl — the
same no-fitting lambda sweep as baselines/reciprocal_lambda_grid_top1ctrl,
scored with the voyage_gemini_ctrl LoRA adapter (top1_ctrl's exact recipe,
retrained on a bigger synthetic batch) instead of top1_ctrl or frozen
voyage-4-nano.
"""
...
from baselines.reciprocal_lambda_grid_voyage_gemini_ctrl.eval import sweep_lambda
...
def test_voyage_gemini_ctrl_adapter_dir_exists() -> None:
    if not (REPO / VOYAGE_GEMINI_CTRL_ADAPTER_DIR).exists():
        import pytest

        pytest.skip("voyage_gemini_ctrl adapter not present in this checkout")
    assert (REPO / VOYAGE_GEMINI_CTRL_ADAPTER_DIR / "adapter_config.json").exists()
