"""Pin twotower_field_pos_bg's copies and its config.
...
and `eval_dev.py` are copies of `twotower_top1_optimised`'s — if they
drift, this arm stops training/selecting the same way `top1_ctrl` did,
breaking the comparison this experiment exists to make.
"""
...
def test_data_py_matches_top1_optimised() -> None:
    copy = REPO / "twotower_field_pos_bg" / "data.py"
...
def test_eval_dev_py_matches_top1_optimised() -> None:
    copy = (REPO / "twotower_field_pos_bg" / "eval_dev.py").read_text(encoding="utf-8")
    orig = (REPO / "twotower_top1_optimised" / "eval_dev.py").read_text(encoding="utf-8")
    normalised = copy.replace("twotower_field_pos_bg", "twotower_top1_optimised")
...
def test_preset_matches_top1_ctrl() -> None:
    ...
    cfg = build_config(
        ...
        output_dir=Path("/tmp/unit_test_field_pos_bg"),
    )
...
def test_row_builder_trims_both_sides_to_positioning_and_background() -> None:
...
def test_row_file_matches_top1_ctrl_population() -> None:
    ...
    path = REPO / "artifacts" / "twotower_field_pos_bg" / "rrf_003_multineg_k1_pos_bg.json"
    if not path.exists():
        import pytest

        pytest.skip("row file not built yet — run scripts/build_rrf_multineg_triplets_pos_bg.py")
    payload = json.loads(path.read_text(encoding="utf-8"))
    summary = payload["summary"]
    assert summary["n_rows"] == 643
    assert summary["n_seekers"] == 297
    assert summary["total_padded_negative_slots"] == 0
    assert summary["seeker_fields"] == ["positioning", "background"]
    assert summary["candidate_fields"] == ["positioning", "background"]
