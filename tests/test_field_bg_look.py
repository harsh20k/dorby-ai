"""Pin twotower_field_bg_look's copies and its config.

Third of three field-pair pinning tests (sibling to test_field_pos_bg.py
and test_field_pos_look.py) — same structure, different field pair.
"""
...
def test_data_py_matches_top1_optimised() -> None:
    copy = REPO / "twotower_field_bg_look" / "data.py"
...
def test_eval_dev_py_matches_top1_optimised() -> None:
    copy = (REPO / "twotower_field_bg_look" / "eval_dev.py").read_text(encoding="utf-8")
    orig = (REPO / "twotower_top1_optimised" / "eval_dev.py").read_text(encoding="utf-8")
    normalised = copy.replace("twotower_field_bg_look", "twotower_top1_optimised")
...
def test_preset_matches_top1_ctrl() -> None:
    ...
    from twotower_field_bg_look.config import build_config

    cfg = build_config(
        "voyage-4-nano",
        run_id="unit_test",
        rows_path=Path("/tmp/rows.json"),
        output_dir=Path("/tmp/unit_test_field_bg_look"),
    )
...
def test_row_builder_trims_both_sides_to_background_and_lookingfor() -> None:
    ...
    text = background_lookingfor(profile)
...
def test_row_file_matches_top1_ctrl_population() -> None:
    ...
    path = REPO / "artifacts" / "twotower_field_bg_look" / "rrf_003_multineg_k1_bg_look.json"
    if not path.exists():
        import pytest

        pytest.skip("row file not built yet — run scripts/build_rrf_multineg_triplets_bg_look.py")
    payload = json.loads(path.read_text(encoding="utf-8"))
    summary = payload["summary"]
    assert summary["n_rows"] == 643
    assert summary["n_seekers"] == 297
    assert summary["total_padded_negative_slots"] == 0
    assert summary["seeker_fields"] == ["background", "lookingFor"]
