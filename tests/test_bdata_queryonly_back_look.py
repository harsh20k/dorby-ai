"""Tests for bdata_queryonly_back_look: lock, packing, ids, encoder drift, no split."""
...
from bdata_queryonly_back_look.encode import cosine_scores as qobl_cosine
from bdata_queryonly_back_look.eval import batched_retrieval_ranks
...
def test_cosine_scores_matches_baselines_voyage_nano():
    ...
    np.testing.assert_allclose(qobl_cosine(a, b), base_cosine(a, b), rtol=0, atol=0)
...
def test_previous_experiment_files_untouched():
    prior = ROOT / "bdata_voyage_nano_posbg" / "encode.py"
    assert prior.is_file()
    text = prior.read_text(encoding="utf-8")
    assert "bdata_queryonly_back_look" not in text
    qobl = ROOT / "twotower_queryonly_back_look" / "eval.py"
    assert qobl.is_file()
    assert "bdata_queryonly_back_look" not in qobl.read_text(encoding="utf-8")
    rrf_store = ROOT / "synth_pipeline" / "pairing_rrf" / "store.py"
    assert rrf_store.is_file()
    assert "bdata_queryonly_back_look" not in rrf_store.read_text(encoding="utf-8")


def test_unique_first_seen_matches_encode_aligned_order():
    from bdata_queryonly_back_look.store import expand_unique, unique_first_seen
...
def test_chroma_roundtrip_isolated_tmp(tmp_path: Path):
    ...
    from bdata_queryonly_back_look.store import (
