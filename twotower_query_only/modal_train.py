"""Modal GPU entrypoint: top1_ctrl's exact recipe, trained on the search
query alone (no profile text at all).
...
image = (
    ...
    .add_local_python_source("twotower_query_only", "twotower", "baselines", "synth_pipeline")
...
def train_remote(
    run_id: str,
    rows_filename: str = "rrf_003_multineg_k1_query_only.json",
    ...
    from twotower_query_only.train import run_training
...
def main(
    ...
    print("=== Modal query-only train finished ===")
    for k, v in result.items():
        print(f"{k}: {v}")
    print(
        f"\nPull artifacts with:\n"
        f"  modal volume get {CHECKPOINT_VOLUME} {run_id} "
        f"./artifacts/twotower_query_only/{run_id}"
    )
