"""LoRA fine-tune on seeker text built from the search query alone.

Trains the exact `top1_ctrl` recipe (see config.py for the full rationale) on
rows whose anchor text is `query_weighted.text.query_only` instead of
`seeker_to_text` — no profile text anywhere in training. Third leg of the
profile/concat/query-only seeker-text comparison alongside
`twotower_top1_optimised/top1_ctrl` (concat) and `twotower_no_query/`
(profile only). Reuses generic, model-agnostic helpers from
twotower.train/twotower.eval read-only; imports nothing from
twotower_top1_optimised, twotower_no_query, or any other experiment package.

Usage (keep train_batch_size * gradient_accumulation_steps == 12 to match
top1_ctrl's 245 optimizer steps):
  python -m twotower_query_only.train --run-id query_only_001 \
      --rows-path artifacts/twotower_query_only/rrf_003_multineg_k1_query_only.json \
...
from twotower_query_only.config import build_config
from twotower_query_only.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_query_only.eval_dev import CorpusRecallDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_query_only/modal_train.py)")
    ...
    # top1_ctrl's library-default loss (scale=20.0, no hardness weighting) —
    # this package changes only the training data, not the loss.
    ...
    try:
        ...
        raise RuntimeError(
            f"MultipleNegativesRankingLoss rejected {loss_kwargs!r} ({exc})."
        ) from exc
    print(f"loss: MultipleNegativesRankingLoss({loss_kwargs})")

    # Same recall@1-aware checkpoint selection top1_ctrl used, so the two
    # models are compared on identically-computed dev/holdout numbers.
    ...
    meta.update(
        {
            "experiment": "twotower_query_only",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    # CLI defaults are generic; modal_train.py's defaults match top1_ctrl's
    # actual launch config (train_batch_size=6, gradient_accumulation_steps=2).
