...
Fully separate from twotower_rrf_triplet/data.py (not imported from it) —
these rows come from the same rrf_003 LLM-judge-labeled synthetic profiles,
never touch a real user, so there is no holdout-leak surface against the real
seed_split; the real 69-pair holdout is only ever touched by
twotower.eval.evaluate_pairs at the end of training.
"""
