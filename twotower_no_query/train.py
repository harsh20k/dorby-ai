"""LoRA fine-tune on seeker text with the search query removed entirely.

Trains the exact `top1_ctrl` recipe (see config.py for the full rationale) on
rows whose anchor text is `profile_to_text` instead of `seeker_to_text` — no
query anywhere in training, as opposed to `twotower_query_weighted/`, which
only swapped text *at eval time* on the already-trained `top1_ctrl` adapter.
Reuses generic, model-agnostic helpers from twotower.train/twotower.eval
read-only; imports nothing from twotower_top1_optimised (a prior run's frozen
code) or any other experiment package.

Usage (keep train_batch_size * gradient_accumulation_steps == 12 to match
top1_ctrl's 245 optimizer steps):
  python -m twotower_no_query.train --run-id no_query_001 \
      --rows-path artifacts/twotower_no_query/rrf_003_multineg_k1_no_query.json \
...
from twotower_no_query.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_no_query.eval_dev import CorpusRecallDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_no_query/modal_train.py)")
