"""Frozen Voyage-4-nano encoder (no LoRA).

Copied from bdata_queryonly_back_look/encode.py then adapter loading removed.
Do not edit that file or baselines/voyage_nano/. Cosine drift pinned by
...
class VoyageNanoEncoder:
    ...
    def __init__(
        ...
        self.cache_dir = (
            Path(cache_dir) if cache_dir else Path("artifacts/bdata_queryonly_back_look_frozen")
        )
        self.cache_dir.mkdir(parents=True, exist_ok=True)

        print(
            f"loading {model_name} on {self.device} "
            f"(max_length={max_length}, truncate_dim={truncate_dim}, frozen=True)"
        )
