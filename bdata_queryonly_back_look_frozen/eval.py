"""All-pairs B-data eval of frozen voyage-4-nano, queryonly_back_look packing.

Seeker = search query only; candidate = background + lookingFor.
No LoRA. No train/holdout split. Retrieval ranks each ACCEPT query
against the unique-candidate document pool.

Control for bdata_queryonly_back_look/ (same packing, LoRA adapter).
"""
...
from bdata_queryonly_back_look_frozen.encode import VoyageNanoEncoder, cosine_scores, pick_device
from bdata_queryonly_back_look_frozen.store import persist_from_eval_arrays


# Quoted from prior runs — different packing and/or population unless noted.
COMPARISON = {
    "queryonly_back_look_001_bdata_lora": {
        "artifact": "artifacts/bdata_queryonly_back_look/metrics.json",
        "population": "same B-data all-pairs; same packing; LoRA adapter",
        "pair_auc": 0.5075,
        "hard_neg_auc": 0.4937,
        "easy_neg_auc": 0.5295,
        "within_seeker_auc": 0.4821,
        "mrr": 0.0514,
        "median_rank": 96.0,
        "recall_at_1": 0.0120,
        "recall_at_5": 0.0678,
        "recall_at_10": 0.1227,
        "recall_at_100": 0.5104,
        "ndcg_at_10": 0.0567,
        "cosine_gap": 0.0020,
        "note": "matched population + packing; this run is the frozen control",
    },
    "queryonly_back_look_001_all200": {
        "artifact": "docs/twotower-queryonly-back-look-experiment.md",
        "population": "200 real seed pairs; same packing + LoRA adapter",
        "pair_auc": 0.5983,
        "hard_neg_auc": 0.6564,
        "mrr": 0.4791,
        "recall_at_1": 0.30,
    },
    "frozen_nano_queryonly_back_look_all200": {
        "artifact": "docs/twotower-queryonly-back-look-experiment.md",
        "population": "200 real seed pairs; same packing; frozen nano",
        "pair_auc": 0.5626,
        "hard_neg_auc": 0.4374,
        "mrr": 0.4763,
        "recall_at_1": 0.28,
        "note": "stage 1 of queryonly_back_look experiment; 178-person corpus",
    },
    "bdata_voyage_nano_posbg_allpairs": {
        ...
        "mrr": 0.0320,
        "recall_at_1": 0.0048,
        "note": "same population, different packing",
    },
...
def run_eval(cfg: ExperimentConfig | None = None) -> dict[str, Any]:
    ...
    print(
        f"model:  {cfg.model_name} (frozen, no LoRA) "
    ...
    return {
        ...
        "notes": {
            ...
            "encoder": "frozen voyage-4-nano; ALL resolved pairs; no LoRA",
            ...
            "vector_db": (
                "isolated Chroma under artifacts/bdata_queryonly_back_look_frozen/chroma/; "
...
def print_bdata_metrics(metrics: dict[str, Any]) -> None:
    pair = metrics["pair"]
    print("\n=== Pair metrics (B-data ALL resolved, frozen queryonly_back_look packing) ===")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="B-data eval of frozen voyage-4-nano (queryonly packing, all pairs)"
    )
