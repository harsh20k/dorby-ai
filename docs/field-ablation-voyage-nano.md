...
**searchQuery was included, unablated, in every run.** Only the candidate side was ablated (one profile field dropped at a time via `baselines/text_field_ablation.py::candidate_to_text_ablate_field`); the seeker side used `seeker_to_text(userContactFile, searchQuery)` unchanged in the baseline and all 8 ablations, so `searchQuery` was always present. This experiment therefore answers "which candidate field does voyage-4-nano rely on when matching against a full seeker profile + query," not "does the query matter at all" — that's a separate, already-answered question (see "Query ablation" in `CLAUDE.md` / the `*_no_query` baseline siblings), which found the query is load-bearing, especially for the Voyage models (retrieval metrics down ~20-26% without it).
