...
| ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`bdata-queryonly-dims012-3d.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/bdata-queryonly-dims012-3d.html) | local (`docs/html/`) | 2026-08-19 | **3D scatter of B-data queryonly LoRA vectors using embedding dims 0/1/2 (not PCA).** Three.js `Points` + OrbitControls; 21,168 unique candidates + 15,821 ACCEPT seekers + 2,483 REJECT seekers. Rebuild: `python scripts/build_bdata_queryonly_dims012_3d.py`. |
| [**`bdata-queryonly-back-look-experiment.html`**](file:///Users/harsh/Artifacts/dorby-ai/docs/html/bdata-queryonly-back-look-experiment.html) | local (`docs/html/`) + [**published**](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/bdata-queryonly-back-look-experiment.html) | **2026-08-18** | **Frozen voyage-4-nano control, same packing, all 18,304 B-data pairs** (added to this page). Pair AUC **0.4869** vs LoRA 0.5075; hard-neg **0.4585** vs 0.4937; MRR **0.0523** vs 0.0514 (tie); R@1 **0.0139** vs 0.0120. LoRA did not uniquely fail — frozen was already chance. Isolated package `bdata_queryonly_back_look_frozen/`. Writeup: `docs/bdata-queryonly-back-look-frozen-experiment.md`. |
| [**`bdata-queryonly-back-look-experiment.html`**](file:///Users/harsh/Artifacts/dorby-ai/docs/html/bdata-queryonly-back-look-experiment.html) | local (`docs/html/`) + [**published**](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/bdata-queryonly-back-look-experiment.html) | **2026-08-18** | **Best 200-pair nano LoRA (`queryonly_back_look_001`) on all 18,304 resolved B-data pairs.** Pair AUC **0.5075** (chance; was 0.5983) — that is the transfer failure. Frozen control on the same packing is 0.4869 / R@1 0.0139. R@1 **0.012** vs 0.30 is mostly pool size (178 → 21,168, 119×): same-quality dilution predicts **0.0025**; observed is ~5× that. Isolated package `bdata_queryonly_back_look/`. Writeup: `docs/bdata-queryonly-back-look-experiment.md`. |
| [**`queryonly-packing-comparison.html`**](file:///Users/harsh/Artifacts/dorby-ai/docs/html/queryonly-packing-comparison.html) | local (`docs/html/`) + [**published**](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/queryonly-packing-comparison.html) | **2026-08-13** | **Four-way all-200 comparison (not a new training run).** Packing 1 (query → full candidate): frozen voyage-4-nano R@1 0.30 / R@10 0.91; `top1_ctrl` eval-time query-only swap is the best retrieval row (R@1 **0.32**, MRR **0.508**, R@5 **0.80**). Packing 2 (query → background+lookingFor): frozen nano hard-neg **0.437** (below chance); `queryonly_back_look_001` trained on that trim hits pair AUC **0.598** and hard-neg **0.656**. Retrieval is the text choice; hard-neg is where training on the trim pays. Writeup: `docs/queryonly-packing-comparison.md`. |
...
| [`project-story.html`](http://dorby-project-story-411960113601.s3-website-us-east-1.amazonaws.com/) | local (`docs/html/`) + S3 static site | 2026-08-13 | Plain-language chronological walkthrough of the whole project (2026-07-16 → 08-13) as a scroll-driven slideshow. Covers the 200-pair seed arc through query-weighting, reciprocal matching, Qwen seed-set records, and the B-data production dump. Not a Claude artifact — hosted on S3, URL recorded in `docs/project-story-url.md`. |
...
| [`twotower-ask-offer-posbg-experiment.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/twotower-ask-offer-posbg-experiment.html) | local (`docs/html/`) + [published](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/twotower-ask-offer-posbg-experiment.html) | 2026-08-12 | **Corrected ask/offer retrain (`twotower_ask_offer_posbg/`, new isolated package):** offer = positioning+background only at train *and* eval, same 3008 rows / λ=1.75 as `ask_offer_001`. All-200 combined AUC **0.5754** vs original wide-offer **0.5714** — a wash. The earlier eval-time swap (0.5572) was train/eval mismatch, not a worse field set. Forward-only still near chance (0.5159); reciprocal still adds ~0.059. Does not beat `voyage_gemini_ctrl` (0.6587). Writeup: `docs/twotower-ask-offer-posbg-experiment.md`. |
| [`twotower-ask-offer-posbg-eval.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/twotower-ask-offer-posbg-eval.html) | local (`docs/html/`) + [published](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/twotower-ask-offer-posbg-eval.html) | 2026-08-12 | **Eval-time offer trim on `ask_offer_001`:** all 200 real pairs, offer = positioning+background only. Combined AUC **0.5572** (was 0.5714). Not a retrain. Writeup: `docs/twotower-ask-offer-posbg-eval-experiment.md`. |
...
| [`bdata-summary.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/bdata-summary.html) | local (`docs/html/`) + [published](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/bdata-summary.html) | 2026-08-12 | **Quantitative overview of locked `data/B-data.json`:** 46,609 seeker×query rows, 15,986 seekers, 58,293 matches. Mostly PENDING (39,989 / 68.6%); among resolved, 15,821 ACCEPT vs 2,483 REJECT (~6.4:1). Channels: BACKGROUND_MATCH 44,796, SMS 12,045, ON_CALL 1,414, AUTOMATCH 38. Built by `scripts/build_summary_B_data.py` (also writes gitignored `data/summary_B_data.html`). |
| [`bdata-voyage-nano-posbg-experiment.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/bdata-voyage-nano-posbg-experiment.html) | local (`docs/html/`) + [published](https://dorby-project-story-411960113601.s3.amazonaws.com/docs/bdata-voyage-nano-posbg-experiment.html) | 2026-08-12 | **Frozen Voyage-4-nano on all resolved B-data pairs, narrower packing:** seeker = lookingFor + query, candidate = positioning + background. No split. Pair AUC **0.5185** (barely above chance); hard-neg AUC **0.4319** (inverted); retrieval MRR **0.032**, median rank 184 of 21,168. Package: `bdata_voyage_nano_posbg/`. Writeup: `docs/bdata-voyage-nano-posbg-experiment.md`. |
