...
| [`experiment-index.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/experiment-index.html) | local (`docs/html/`) | 2026-07-20, updated 2026-08-09 | Chronological index of every visualization this project has produced (published Claude artifacts + local self-contained HTML), most recent first |
...
| [`voyage-nano-field-sweep-heatmap.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/voyage-nano-field-sweep-heatmap.html) | local (`docs/html/`) | 2026-08-09 | 105-combo grid search over seeker/candidate field subsets (positioning/background/lookingFor) x search-query inclusion for Voyage-4-nano, plus a query-only seeker (no profile fields) x the same 7 candidate subsets. Interactive: pick fields to see a combo's exact metrics, rank all combos by any of 12 tracked metrics for a top-3 summary, or explore a metric-selectable 8x7 heatmap (seeker rows incl. "None" x candidate columns). **Best combo (seeker=positioning, candidate=lookingFor, query=yes) reaches 0.6110 pair AUC — beats the full-profile baseline (0.5614) and the LLM-judge-borrowed field selection (0.5658), but the field selection that wins for an embedding model is not the one that wins for the LLM judge (which tops out at 0.6451 on the same population). A query-only seeker still beats the full-profile baseline (0.5861 pair AUC with zero seeker fields).** No holdout check — see `scripts/build_voyage_nano_field_sweep_browser.py` / `docs/voyage-nano-field-sweep-experiment.md` |
| Voyage-4-nano field/query sweep | [published](https://claude.ai/code/artifact/1e698f4e-15a6-44b4-bd76-2381befb2f40) | 2026-08-09 | Published version of `voyage-nano-field-sweep-heatmap.html` above |
| [`twotower-top1-ctrl-field-sweep-heatmap.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/twotower-top1-ctrl-field-sweep-heatmap.html) | local (`docs/html/`) | 2026-08-09 | The identical 105-combo grid sweep above, re-run against `top1_ctrl` (this project's best fine-tuned model, a LoRA adapter on Voyage-4-nano) instead of the frozen encoder. Same interactive controls (explore-a-combo, top-3-by-metric, metric-selectable heatmap). **Best combo (seeker=background, candidate=lookingFor, query=yes) reaches 0.6395 pair AUC — beats the full-profile top1_ctrl baseline (0.5683), the frozen-model sweep's own best (0.6110), and every other embedding-family model in the project's all-200 comparison, landing within 0.006 of the LLM judge's own best (0.6451) at zero extra serving cost over frozen nano.** No holdout check — see `scripts/build_twotower_top1_ctrl_field_sweep_browser.py` / `docs/twotower-top1-ctrl-field-sweep-experiment.md` |
| top1_ctrl field/query sweep | [published](https://claude.ai/code/artifact/4ca20e7d-1bcd-4972-a4b5-9d2709f3d300) | 2026-08-09 | Published version of `twotower-top1-ctrl-field-sweep-heatmap.html` above |
| [`twotower-queryonly-back-look-field-sweep-heatmap.html`](file:///Users/harsh/Artifacts/dorby-ai/docs/html/twotower-queryonly-back-look-field-sweep-heatmap.html) | local (`docs/html/`) | 2026-08-09 | The identical 105-combo grid sweep above, re-run against `queryonly_back_look_001` — the new best two-tower fine-tune in the project on every tracked metric, beating `top1_ctrl` (`docs/twotower-queryonly-back-look-experiment.md`). Same interactive controls (explore-a-combo, top-3-by-metric, side-by-side query-included/excluded heatmaps, 12-metric mini-grid). **The sweep's own row for the training combo reproduces the canonical eval exactly (0.5983 pair AUC), but the grid's overall best (seeker=positioning, candidate=lookingFor, query=yes, 0.6349 pair AUC) lands slightly below `top1_ctrl`'s own grid best (0.6395) — fine-tuning on one text representation raised the ceiling locally, not across the whole field/query landscape.** No holdout check — see `scripts/build_twotower_queryonly_back_look_field_sweep_browser.py` / `docs/twotower-queryonly-back-look-field-sweep-experiment.md` |
| queryonly_back_look field/query sweep | [published](https://claude.ai/code/artifact/b9baf2d7-e7de-4962-918b-a3fe48b7ccf2) | 2026-08-09 | Published version of `twotower-queryonly-back-look-field-sweep-heatmap.html` above |
