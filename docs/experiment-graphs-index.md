...
| [`twotower-no-query-comparison.html`](https://claude.ai/code/artifact/8d467198-ce99-4dc1-9008-a0f4910ecf8c) | published artifact + local (`docs/html/`) | 2026-08-03 | **Does retraining `top1_ctrl` with the search query removed from training entirely replicate the eval-time query-weighting win found in `twotower_query_weighted/`? No.** New isolated package `twotower_no_query/`, exact `top1_ctrl` recipe (library-default loss, `recall@1` checkpoint selection, micro-batch 6/accum 2, lr 2e-4, 5 epochs, A100-80GB) retrained on rows whose anchor text is `profile_to_text` instead of `seeker_to_text` — verified row-for-row identical to the original k1 file except the anchor text (all 643 rows), confirmed via a zero-cost local `--dry-run` (identical 583/60 split, identical LoRA target counts) before any GPU spend. All-200 real pairs: `no_query_001` scores AUC 0.5718 / MRR 0.3371 / **R@1 0.18** / R@10 0.67 — the weakest of four rows compared and effectively identical to the **untrained** frozen baseline (R@1 0.18), versus `top1_ctrl`'s own concat-trained R@1 0.19 and the eval-time `query_only`/`alpha_0.6` tricks' 0.32/0.29. Training the model to never see the query at all does not recover the eval-time trick's gain — it erases the fine-tune's contribution entirely. **The 69-pair holdout misled again** (4th+ time in this project): `no_query_001`'s holdout R@1 was 0.4138, MRR 0.5574 — looked like the best fine-tune ever, reversed completely on all 200. Writeup: `docs/twotower-no-query-experiment.md` |
