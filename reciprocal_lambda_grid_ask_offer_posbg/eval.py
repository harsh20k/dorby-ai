"""Lambda sensitivity sweep for the ask_offer_posbg_001 jointly-trained two-tower model.

Deliberate duplicate of reciprocal_lambda_grid_ask_offer/eval.py's sweep
mechanics (experiment isolation rule), with two substitutions:

1. Adapters come from ``ask_offer_posbg_001`` (volume
   ``dorby-twotower-ask-offer-posbg-checkpoints``), not ``ask_offer_001``.
2. Offer text is ``positioning`` + ``background`` only
   (``twotower_ask_offer_posbg.text``), not the wide field set
   ``baselines.reciprocal_static.text`` used by the original ask_offer sweep.

Ask text is unchanged (lookingFor + seeker searchQuery). The model itself
was trained at a fixed lambda=1.75 with this same packing; this module
reuses its encode/text plumbing read-only and adds the missing piece:
sweeping lambda over a grid post-hoc, the same no-fitting diagnostic every
other reciprocal_lambda_grid* package runs.
...
Usage:
  python -m reciprocal_lambda_grid_ask_offer_posbg.eval \\
      --adapter-dir artifacts/twotower_ask_offer_posbg/ask_offer_posbg_001/adapter
"""
...
from twotower_ask_offer_posbg.config import TOWER_CONFIG
...
def run_eval(
    ...
    return {
        "model_name": "voyageai/voyage-4-nano + ask_offer_posbg_001 two LoRA towers (ask + offer, pos+bg)",
        "adapter_dir": str(adapter_dir),
        "device": str(device),
        "batch_size": batch_size,
        "ask_fields": ["lookingFor", "searchQuery (seeker only)"],
        "offer_fields": ["positioning", "background (twotower_ask_offer_posbg.text.BG_FIELDS)"],
        "note": (
            "No fitting step: every lambda in the grid is evaluated directly "
            "against these same labels. ask_offer_posbg_001 itself was trained with "
            "a fixed lambda=1.75 baked into its loss and with offer = positioning+"
            "background only; this sweep is a post-hoc diagnostic on top of that, "
            "not a fit or a value chosen for deployment."
        ),
        "subsets": curves,
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Lambda sensitivity sweep on ask_offer_posbg_001 (no fitting)")
    ...
    p.add_argument("--artifacts-dir", type=Path, default=Path("artifacts/reciprocal_lambda_grid_ask_offer_posbg"))
