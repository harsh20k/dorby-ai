...
Usage (all three arms keep train_batch_size * gradient_accumulation_steps = 12):
  # Arm A — batch only
  python -m twotower_rrf_triplet_ablation.train --run-id abl_a_batch_only \
  ...
  python -m twotower_rrf_triplet_ablation.train --run-id abl_b_negs_only \
      --rows-path artifacts/twotower_rrf_triplet_ablation/rrf_003_multineg_k2.json \
      --negatives-per-anchor 2 --train-batch-size 2 --gradient-accumulation-steps 6
  # Arm C — baseline corner
  python -m twotower_rrf_triplet_ablation.train --run-id abl_c_baseline \
      --rows-path artifacts/twotower_rrf_triplet_ablation/rrf_003_multineg_k1.json \
...
from twotower_rrf_triplet_ablation.data import carve_dev, load_multineg_rows, rows_to_hf_dict
from twotower_rrf_triplet_ablation.eval_dev import MultiNegTripletDevEvaluator
...
def run_training(
    ...
    *,
    ...
    device = pick_device()
    if device == "mps":
        print("warning: MPS selected — prefer Modal CUDA (twotower_rrf_triplet_ablation/modal_train.py)")
    ...
    meta.update(
        {
            "experiment": "twotower_rrf_triplet_ablation",
